    ...
    public class AdminDashboardService : IAdminDashboardService
    {
        ...
        public async Task<object> GetStatsOverviewAsync(string section, CancellationToken ct)
        {
            try
            {
                ...
                var dailyRevenue = await _context.PaymentsPayments.Where(p => p.PaymentDate == todayIstDate && p.Status != null && p.Status.ToLower() == "verified").SumAsync(p => (decimal?)p.Amount, ct) ?? 0;
                var monthlyRevenue = await _context.PaymentsPayments.Where(p => p.PaymentDate >= monthIstDate && p.PaymentDate <= todayIstDate && p.Status != null && p.Status.ToLower() == "verified").SumAsync(p => (decimal?)p.Amount, ct) ?? 0;
            ...
            catch
            {
                return new
                {
                    ...
                    payments = new { month_amount = 0, month_count = 0 },
                    attendance = new { today_present = 0, today_absent = 0, today_pending = 0 },
                    seats = new { total = 0, available = 0, occupied = 0 }
                };
            }
        }
        ...
        public async Task<object> GetDashboardChartsAsync(string range, CancellationToken ct)
        {
            try
            {
                ...
                var paymentGroups = await _context.PaymentsPayments
                    .Where(p => p.PaymentDate >= minMonthDateOnly && p.Status != null && p.Status.ToLower() == "verified")
            ...
            catch
            {
                return new
                {
                    ...
                    revenue_trend = new { labels = new System.Collections.Generic.List<string>(), data = new System.Collections.Generic.List<decimal>() },
                    study_hours = new { labels = new System.Collections.Generic.List<string>(), data = new System.Collections.Generic.List<double>() },
                    seat_occupancy = new { labels = new[] { "Available", "Occupied" }, data = new[] { 0, 0 } }
                };
            }
        }
